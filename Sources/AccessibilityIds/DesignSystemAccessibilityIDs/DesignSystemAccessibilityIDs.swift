//
//  DesignSystemAccessibilityIDs.swift
//
//
//  Created by user on 24.06.2024.
//

public class DesignSystemAccessibilityIDs {
    public class LabelView {
        public static let id = "label_view"
        public static let labelView = "label_view_label_view"
        public static let container = "label_view_container"
    }
    
    public class SegmentItemView {
        public static let id = "segment_item_view_"
        public static let containerView = "segment_item_container_view"
        public static let label = "segment_item_view_label"
    }
    
    public class SegmentControl {
        public static let id = "segment_control_view"
        public static let containerView = "segment_control_container_view"
        public static let segmentStack = "segment_control_stack"
    }
    
    public class InputView {
        public static let id = "input_view"
        public static let label = "input_view_label"
        public static let hint = "input_view_hint"
        public static let textField = "input_view_text_field"
        public static let verticalStack = "input_view_stack"
        public static let textFieldContainer = "input_view_text_field_container"
    }
    
    public class ButtonView {
        public static let id = "button_view"
        public static let label = "button_label"
    }
    
    public class TitleView {
        public static let id = "title_view"
        public static let label = "title_view_label"
    }
    
    public class InputOTPItemView {
        public static let id = "input_otp_item_view_"
        public static let label = "input_otp_item_view_label"
    }
    
    public class InputOTPView {
        public static let id = "input_otp_view"
        public static let itemsStack = "input_otp_items_stack"
    }
}
